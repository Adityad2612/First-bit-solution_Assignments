# 🚀 FirstBit Solution – Project Portfolio

Welcome! This repository contains multiple console-based projects.  
Click any project from the list below to view full documentation.

---

## 📌 Available Projects

🔗 **Book Management System (C Programming)**  
➡ [Open Project README](./readme_c.md)

🔗 **Banking Management System (Java – Core & Arrays)**  
➡ [Open Project README](./redeme1_java.md)

---

## 📝 About
This repository contains academic + practical projects developed while learning C and Java.
Each project includes complete code and documentation for better understanding.

---

## 💡 Upcoming Projects (Optional)
You can add more here later:

- ATM Simulation (Java)
- Student Result Management (C)
- Movie Ticket Booking (Java)

---

📌 *Click any project above to explore source code and documentation.*
